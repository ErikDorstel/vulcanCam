...
const char* ssidAP="vulcanCam"; const char* passwordAP="";
String ssidStation=""; String passwordStation=""; boolean statusStation=false;

void WiFiStationConnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  preferences.begin("vulcanCam",false); preferences.putString("ssidStation",ssidStation); preferences.putString("passwordStation",passwordStation); preferences.end();
...
void WiFiStationDisconnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  preferences.begin("vulcanCam",false); String ssidStationOld=preferences.getString("ssidStation",""); String passwordStationOld=preferences.getString("passwordStation",""); preferences.end();
  if (ssidStationOld==ssidStation & passwordStationOld==passwordStation & WiFi.softAPgetStationNum()==0) { WiFi.reconnect(); } }

void initWLAN() {
  preferences.begin("vulcanCam",false); ssidStation=preferences.getString("ssidStation",""); passwordStation=preferences.getString("passwordStation",""); preferences.end();
  ...
  WiFi.setHostname("vulcanCam"); WiFi.begin(ssidStation.c_str(),passwordStation.c_str());
